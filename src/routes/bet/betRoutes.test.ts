...
describe("Bet Routes", () => {
  ...
  describe("POST /api/bet/place", () => {
    it("should return 400 for invalid bet amount", async () => {
      const response = await request(app).post("/api/bet/place").send({ amount: 0 });
    ...
    it("should return 400 for negative bet amount", async () => {
      const response = await request(app).post("/api/bet/place").send({ amount: -10 });
    ...
    it("should return 400 for missing bet amount", async () => {
      const response = await request(app).post("/api/bet/place").send({});
    ...
    it("should successfully place a bet", async () => {
      ...
      const response = await request(app).post("/api/bet/place").send({ amount: 5 });
  ...
  describe("POST /api/bet/many-spins", () => {
    it("should return 400 for invalid bet amount", async () => {
      const response = await request(app).post("/api/bet/many-spins").send({ amount: 0 });
    ...
    it("should return 400 for invalid number of spins", async () => {
      const response = await request(app).post("/api/bet/many-spins").send({ amount: 5, spins: 0 });

      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty(
        "error",
        "Invalid number of spins. Must be between 1 and 10000000"
      );
      expect(mockRunManySpins).not.toHaveBeenCalled();
    });
    ...
    it("should successfully run spins with default count", async () => {
      ...
      const response = await request(app).post("/api/bet/many-spins").send({ amount: 5 });
