...
function validateOutcomeWeights(weights: OutcomeWeights): void {
  ...
  if (keys.length !== 3 || !requiredKeys.every((k) => keys.includes(k))) {
    throw new InvalidWeightsError(
      "All outcomeWeights keys (threeOfAKind, twoOfAKind, noWin) must be present."
    );
  }
...
function validateSymbolWeights(weights: SymbolWeights): void {
  ...
  if (
    keys.length !== requiredIndexes.length ||
    !requiredIndexes.every((idx) => keys.includes(idx))
  ) {
    throw new InvalidWeightsError(
      `symbolWeights must have all indexes from 0 to ${SLOT_SYMBOLS.length - 1}.`
    );
  }
...
export class BetService {
  ...
  private determineWinType(outcomeWeights?: OutcomeWeights): WinType {
    ...
    const wBust =
      (this.symbols.length - 1) * (this.symbols.length - 2) * (outcomeWeights?.noWin ?? 1);
  ...
  private selectWinningSymbol(symbolWeights?: SymbolWeights): number {
    ...
    const symbolWeightsArray = this.symbols.map((_, idx) => symbolWeights?.[idx] ?? 1);
  ...
  private generateTwoOfAKindSymbols(winningSymbol: number): {
    symbols: number[];
    winningSymbol: number;
  } {
    const availableIndices = [...Array(this.symbols.length).keys()].filter(
      (i) => i !== winningSymbol
    );
    const randomIndex = Math.floor(Math.random() * availableIndices.length);
  ...
  private generateThreeOfAKindSymbols(winningSymbol: number): {
    symbols: number[];
    winningSymbol: number;
  } {
    return {
  ...
  ): { symbols: number[]; winningSymbol: number } {
    if (winType === WinType.NO_WIN) {
  ...
  public placeBet(request: BetRequest): BetResponse {
    ...
    if (autowin) {
      ...
      const { symbols, winningSymbol: generatedWinSymbol } = this.generateSymbols(
        winType,
        symbolWeights
      );
    ...
    const winAmount = this.calculateWinAmount(
      SYMBOL_VALUES[this.symbols[winSymbol]],
      betAmount,
      winType
    );
